                        ...
                        <div class="login-wrapper__body">
                            <form action="{{ route('admin.login') }}" method="POST"
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" @if(isset($_COOKIE['admin_email'])) value=" {{ $_COOKIE['admin_email'] }}" @endif name="username" required>
                                </div>
                                <div class="form-group">
                                    <label>@lang('Password')</label>
                                    <input type="password" class="form-control" @if(isset($_COOKIE['admin_password'])) value="{{ $_COOKIE['admin_password'] }}" @endif name="password" required>
                                </div>
                                <x-captcha />
                                <div class="d-flex flex-wrap justify-content-between">
                                    <div class="form-check me-3">
                                        <input class="form-check-input" name="remember" type="checkbox" @if(isset($_COOKIE['admin_email'])) checked @endif id="remember">
